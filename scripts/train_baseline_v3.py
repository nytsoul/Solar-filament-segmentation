...
def plot_history(history, outputs_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(outputs_dir, 'training_curves_v3.png'))
    plt.close()
...
def main():
    ...
    experiment_dir = os.path.join(out_dir_base, 'outputs', 'full_image_baseline_v3')
    checkpoints_dir = os.path.join(out_dir_base, 'checkpoints')
    os.makedirs(experiment_dir, exist_ok=True)
    os.makedirs(checkpoints_dir, exist_ok=True)

    # Setup logging
    log_path = os.path.join(experiment_dir, 'training_v3.log')
    logging.basicConfig(
        level=logging.INFO,
    ...
    # Save config
    with open(os.path.join(experiment_dir, 'config_v3.yaml'), 'w') as f:
    ...
    tversky_alpha = loss_cfg.get('tversky_alpha', 0.7)
    tversky_beta = loss_cfg.get('tversky_beta', 0.3)
    tversky_smooth = loss_cfg.get('tversky_smooth', 1.0)
    focal_alpha = loss_cfg.get('focal_alpha', 0.25)
    focal_gamma = loss_cfg.get('focal_gamma', 2.0)
    
    logger.info(f"Loss config: Focal(alpha={focal_alpha}, gamma={focal_gamma}) + Tversky(alpha={tversky_alpha}, beta={tversky_beta})")
    tversky_fn = TverskyLoss(alpha=tversky_alpha, beta=tversky_beta, smooth=tversky_smooth)

    def combined_loss(y_pred, y_true):
        focal = sigmoid_focal_loss(y_pred, y_true, alpha=focal_alpha, gamma=focal_gamma, reduction='mean')
        tversky = tversky_fn(y_pred, y_true)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(train_iter, desc=f"Epoch {epoch}/{epochs} [Train]", total=total_train):
            ...
            with torch.no_grad():
                ...
                train_prob_sum += probs.sum().item()
                train_fg_sum += (probs > 0.5).sum().item()
                total_pixels += probs.numel()

        train_loss /= total_train
        epoch_mean_prob = train_prob_sum / total_pixels
        epoch_fg_pct = (train_fg_sum / total_pixels) * 100
        ...
        logger.info(f"Epoch {epoch} | LR: {current_lr:.6f} | Train Loss: {train_loss:.4f} | "
                     ...
                     f"Train Prob Mean: {epoch_mean_prob:.4f} | Train FG%: {epoch_fg_pct:.2f}% | "
        ...
        epoch_data = {
            ...
            "train_prob_mean": epoch_mean_prob,
            "train_fg_pct": epoch_fg_pct,
        ...
        if val_dice > best_dice:
            ...
            best_epoch = epoch
            best_iou = val_iou
            torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'best_baseline_v3.pth'))
            logger.info("  --> Saved new best model (Full-Image Validation Dice)")

        torch.save(model.state_dict(), os.path.join(checkpoints_dir, 'latest_checkpoint_v3.pth'))
        ...
        pd.DataFrame(history_serializable).to_csv(os.path.join(experiment_dir, 'training_history_v3.csv'), index=False)
        with open(os.path.join(experiment_dir, 'training_history_v3.json'), 'w') as f:
    ...
    final_metrics = {
        "experiment": "full_image_baseline_v3",
        ...
        "amp_used": use_amp,
        "best_checkpoint_path": os.path.join(checkpoints_dir, 'best_baseline_v3.pth'),
        "sampling": {
    ...
    with open(os.path.join(experiment_dir, 'final_metrics_v3.json'), 'w') as f:
